...
/*──────────────────────────────────────────────────────────*/
/*  Adelia– single‑file image board (2025‑06‑20)  */
/*   Requires PHP ≥ 8.4.0                                    */
/*──────────────────────────────────────────────────────────*/
...
const
    ...
    PAGE_SIZE   = 15,                // threads per index page
...
function handleUpload(array $file): array
{
    ...
    $data   = file_get_contents($file['tmp_name'], false, null, 0, 256); // peek
...
if ($action === 'mod_delete' && isset($_POST['id'], $_POST['pw'])) {
    ...
    if (!password_verify($_POST['pw'], MOD_HASH)) {
        http_response_code(403); exit('bad password');
    }
    $id = (int)$_POST['id'];
    $pdo = db();
    $row = $pdo->prepare('SELECT image, thumb FROM posts WHERE id=?');
    $row->execute([$id]);
...
if ($action === 'post') {
    ...
    if ($parent) {
        $pdo->prepare('UPDATE posts SET bumped_at=? WHERE id=?')->execute([$now,$parent]);
    }
    header('Location: '.($parent ? "?thread=$parent" : './'));
    exit;
}
...
if ($tid) {                               /* Thread view with pagination */
    $stmt = $pdo->prepare('SELECT COUNT(*) FROM posts WHERE parent=?');
    $stmt->execute([$tid]);
    $totalReplies = (int)$stmt->fetchColumn();
    $pages = max(1, (int)ceil($totalReplies / PAGE_SIZE));
    ...
    $pages = max(1, (int)ceil($totalThreads / PAGE_SIZE));
    $offset = ($page - 1) * PAGE_SIZE;
...
<form method="post" enctype="multipart/form-data">
  ...
  <table>
    <tr><th>Name</th><td><input name="name" size="25" maxlength="35" required></td></tr>
    <tr><th>Comment</th><td>
...
<?php foreach ($posts as $p): ?>
  ...
  <div class="post <?= $p['parent']?'reply':'op' ?>" id="p<?= $p['id'] ?>">
    ...
    <div class="body"><?= nl2br(h($p['body'])) ?></div>
...
<form method="post" enctype="multipart/form-data">
  ...
  <table>
    <tr><th>Name</th><td><input name="name" size="25" maxlength="35" required></td></tr>
    <tr><th>Subject</th><td><input name="subject" size="25" maxlength="100"></td></tr>
    <tr><th>Comment</th><td>
      <textarea name="body" rows="5" cols="35" required></textarea>
...
<?php foreach ($posts as $op): ?>
  <div class="thread" id="thread_<?= $op['id'] ?>">
    ...
    <div class="post op" id="op_<?= $op['id'] ?>">
      ...
      <div class="body"><?= nl2br(h($op['body'])) ?></div>
    </div>
<?php
...
?>
    <div class="post reply preview" id="p<?= $rp['id'] ?>">
      ...
      <div class="body"><?= nl2br(h($rp['body'])) ?></div>
    </div>
<?php endforeach; ?>
    <br class="clear"><hr>
  </div>
